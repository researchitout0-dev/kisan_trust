...
def _get_client():
    ...
    if _client is None:
        ...
        print("[diagnosis_service] Gemini client ready.")
    return _client
...
def generate_diagnosis(uploaded_url: str, healthy_url: str) -> str:
    """
    Send both images to Gemini and get a morphological diagnosis.

    Args:
        uploaded_url:  URL of the farmer's uploaded crop image
        healthy_url:   URL of a healthy reference image for the same crop
    ...
    # Build multimodal content for Gemini using the new SDK
    contents = [
